        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-3">
          {signals.map((signal, i) => (
            <Reveal key={signal.id} delay={i * 0.08}>
              <article className="group relative h-full overflow-hidden rounded-lg border border-border bg-surface p-6 transition-colors duration-300 hover:border-orange">
                <span
                  aria-hidden
                  style={{ clipPath: OCTAGON }}
                  className="flex h-9 w-9 items-center justify-center border border-orange-deep/50 text-sm font-bold text-orange-deep transition-colors duration-300 group-hover:border-orange group-hover:bg-orange group-hover:text-[#431407]"
                >
                  ?
                </span>
